...
class LabReader extends FileReader<Map<String, ClassSchedule>> {
    ...
    private static var onlineFormat:EReg = ~/^\s*DISTANCE PROGRAM/;
    
    private var relevantLabs:Array<String>;
    private var courses:Map<String, ClassSchedule>;
 ...
 */
     ...
     *  Rince and repeat.
     */
        courses = new Map<String, ClassSchedule>();
        ...
        try {
            ...
            while (true) {
                ...
                else if (onlineFormat.match(line))
                    addToCourses(curSchedule);
    ...
    private function addToCourses(schedule:ClassSchedule):Void {
        if (!this.courses.exists(schedule.toString()) && this.relevantLabs.indexOf(schedule.code) >= 0)
            courses.set(schedule.toString(), schedule);
    }
